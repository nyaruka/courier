...
func (s *StatusUpdate) ExternalIdentifier() string {
	if s.ExternalIdentifier_ != "" {
		return s.ExternalIdentifier_
	}
	return s.ExternalID_
}
func (s *StatusUpdate) SetExternalIdentifier(id string) {
	s.ExternalID_ = id
	s.ExternalIdentifier_ = id
}
...
UPDATE msgs_msg SET 
    ...
    FROM 
        (VALUES(:msg_uuid::uuid, :channel_id::int, :status, :external_id, :external_identifier, :log_uuid::uuid)) AS s(msg_uuid, channel_id, status, external_id, external_identifier, log_uuid),
        contacts_contact c

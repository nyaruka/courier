...
func (h *handler) processFacebookInstagramPayload(ctx context.Context, channel courier.Channel, payload *handlers.WACMOPayload, w http.ResponseWriter, r *http.Request, clog *courier.ChannelLog) ([]courier.Event, []interface{}, error) {
	var err error
...
func requestWAC(payload handlers.WACMTPayload, accessToken string, status courier.MsgStatus, wacPhoneURL *url.URL, zeroIndex bool, clog *courier.ChannelLog) (courier.MsgStatus, error) {
	jsonBody, err := json.Marshal(payload)
	if err != nil {
		return status, err
	}

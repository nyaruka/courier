...
func (h *handler) Send(ctx context.Context, msg courier.MsgOut, res *courier.SendResult, clog *courier.ChannelLog) error {
	...
	callbackURL := fmt.Sprintf("https://%s/c/%s/%s/status?id=%s&action=callback", callbackDomain, strings.ToLower(string(h.ChannelType())), msg.Channel().UUID(), string(msg.UUID()))

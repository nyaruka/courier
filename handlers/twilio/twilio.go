...
func (h *handler) StatusMessage(ctx context.Context, channel courier.Channel, w http.ResponseWriter, r *http.Request) ([]courier.Event, error) {
	...
	if !found {
		return nil, courier.WriteRequestError(ctx, w, r, channel, fmt.Errorf("unknown status '%s', must be one of 'queued', 'failed', 'sent', 'delivered', or 'undelivered'", twStatus.MessageStatus))
	}
	...
	if h.ignoreDeliveryReports && msgStatus != courier.MsgFailed {
		return nil, courier.WriteRequestIgnored(ctx, w, r, channel, "ignoring non error delivery report")
	}
